...
import GolFactory from '../../build/GolFactory.json'
...
export async function factoryFixture([wallet]: Wallet[], _: Web3Provider): Promise<FactoryFixture> {
  const factory = await deployContract(wallet, GolFactory, [wallet.address], overrides)
  return { factory }
}
...
export async function pairFixture([wallet]: Wallet[], provider: Web3Provider): Promise<PairFixture> {
  ...
  const pair = new Contract(pairAddress, JSON.stringify(GolPair.abi), provider).connect(wallet)
